...
> M320「暂停」**显示但不记录**（不做边沿落库）：其他报警是 M320 的触发条件，任一其他报警发生时 M320 必然为 TRUE，记日志没有额外信息。它仍在诊断页当前报警文本框中实时显示。FC01 读取范围不变（仍覆盖 M300~M401）。

- 报警线圈按 `m_coil(300)` 起、连续区间读取；10 个报警点均实时显示，其中 9 点做边沿检测与落库（M320 暂停只显示不记），提示位 M400/M401 只展示不记日志。
...
1. 功能码 01 **一次**读 `M300~M401` 共 102 个连续线圈（Modbus 单次读线圈上限远大于此），在结果中按下标只取实际使用的 10 个报警位（M300~M303/M310~M312/M320~M322；其中 M320 暂停只显示不记日志，落库 9 点）与 2 个提示位（M400/M401），中间空洞忽略；相比分两次读区间，省一次事务。

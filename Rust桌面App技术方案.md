...
**心跳寄存器（常驻写，第 8 条例外）**：`D210 随机数`（INT，变量名 HMI_RDV1）——与 PLC 连接成功后，无论当前在哪个界面，后端每 **100ms** 写入一个新随机数（功能码 06），供 PLC 侧判断上位机在线；断开连接即停。
...
| 胸背轴工作减速度 | D2288 | 16 | D2318 | REAL | 最大 743.8 |
...
| CMD 脉冲还是电平触发不确定 | 对照 FB_ECAxis / FB_Pusher 逻辑，脉冲宽度可配 |
...
| 连接/读写/重连卡住界面线程 | 全部 tokio 异步 + 超时，命令 async 非阻塞；同步 SQLite 走 spawn_blocking（第 10 条） |
| WebView2 缺失 | 安装包集成引导 |
...
| [YEDASAVG/vyuh_hmi](https://github.com/YEDASAVG/vyuh_hmi) | Flutter + Rust WebSocket + Modbus TCP | 工业 HMI 实时监控 PLC | HMI 数据点建模、实时刷新模式（前端框架不同，仅借思路） |
